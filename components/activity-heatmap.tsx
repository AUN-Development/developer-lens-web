...
// Kleuren gebaseerd op het lime-accent uit je design
const legend = [
  { label: "0", className: "bg-muted" },
  { label: "1-2", className: "bg-[#C1FF72]/20" },
  { label: "3-5", className: "bg-[#C1FF72]/50" },
  { label: "6-9", className: "bg-[#C1FF72]/80" },
  { label: "10+", className: "bg-[#C1FF72]" },
];
...
export function ActivityHeatmap({
  ...
  const weeks = Array.from({ length: columns }, (_, weekIndex) =>
    Array.from({ length: 7 }, (_, dayIndex) =>
      filteredDays.find((day) => day.weekIndex === weekIndex && day.dayIndex === dayIndex),
    ),
  );
    ...
    <div className="w-full space-y-4">
      ...
      <div className="flex items-center justify-end gap-2 text-[9px] font-bold uppercase tracking-widest text-muted-foreground">
        ...
        <div className="flex gap-1">
          {legend.map((entry, i) => (
            <div key={i} className={`h-2.5 w-2.5 rounded-xs ${entry.className} border border-black/5 dark:border-white/5`} />
          ))}
        </div>
        <span>More</span>
      </div>
          ...
          {weeks.map((week, weekIndex) => (
            <div key={weekIndex} className="grid grid-rows-7 gap-0.75 sm:gap-1">
              {week.map((day, dayIndex) => {
                if (!day) return <div key={`${weekIndex}-${dayIndex}-empty`} className="h-2 w-2" />;

                const clickable = canClickDay ? canClickDay(day) : true;

                const total = filter === "commits" ? day.commits :
                  filter === "pull-requests" ? day.pullRequests :
                    day.commits + day.pullRequests;

                const intensity = total === 0 ? 0 : getIntensity({ ...day, commits: total, pullRequests: 0 });
                ...
                return (
                  <button
                    ...
                    className={`h-2 w-2 rounded-xs transition-all duration-300 sm:h-2.5 sm:w-2.5 ${bgClasses[intensity]} ${active ? "scale-125 z-10" : ""
